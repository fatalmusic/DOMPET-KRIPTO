	...
	if($_SERVER["REQUEST_METHOD"] == "POST") {
		...
		if(!empty($_POST["token"])) {
			if($helper->verifySession($_POST["token"])) {
				echo json_encode(array("message" => "tunggu sebentar...", "valid" => true, "username" => $helper->username));
			} else {
				echo json_encode(array("error" => "gagal.", "valid" => false));
			}
		} else {
			...
			if($helper->verifyPassword($password)) {
				...
				echo json_encode(array("message" => "tunggu sebentar...", "token" => $token, "valid" => true, "username" => $helper->username));
			} else {

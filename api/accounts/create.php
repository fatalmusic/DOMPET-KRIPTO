<?php
	...
	if($_SERVER["REQUEST_METHOD"] == "POST") {
		...
		if($helper->verifySession($token)) {
			if($helper->username == "admin") {
				if($helper->validUsername($account)) {
					...
					echo json_encode(array("message" => "akun telah dibuat."));
				} else {
					echo json_encode(array("error" => "username hanya menggunakan karakter dan angka."));
				}
			} else {
		...
		echo json_encode(array("error" => "gunakan POST."));
	}
?>